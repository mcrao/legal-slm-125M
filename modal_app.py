...
# ---- Phase 5: pretrain the 125M model on 8xH100 (torchrun DDP) ----
gpu_image = (
    modal.Image.debian_slim(python_version="3.12")
    .pip_install(
        "torch==2.5.1",
...
@app.function(image=gpu_image, gpu=f"{config.PRETRAIN_GPU}:{config.PRETRAIN_GPU_COUNT}",
              volumes=VOLUMES, timeout=60 * 60 * 4)
def pretrain(epochs: float = 2.0, smoke_steps: int = 0, compile: bool = False) -> None:
    import os
    import subprocess
    ...
    subprocess.run(
        ["torchrun", "--standalone",
         f"--nproc_per_node={config.PRETRAIN_GPU_COUNT}", "/root/train.py"],
        check=True, env=env,
    )


@app.function(image=gpu_image, gpu="L4", volumes=VOLUMES, timeout=60 * 20)
def evaluate_model(n_gen_tokens: int = 120) -> dict:
    import glob
    import math
    ...
    with torch.no_grad():
        for f in sorted(glob.glob(f"{config.VAL_TOKENS_DIR}/*.bin")):
            ...
            for i in range(0, nwin, 32):
                ...
                ntok = x.numel() - x.shape[0]  # (seq-1) predicted tokens per row
                total_loss += loss.item() * ntok
                total_tok += ntok
    ppl = math.exp(total_loss / total_tok)
    print(f"\nFULL VAL perplexity: {ppl:.3f}  (loss {total_loss/total_tok:.4f}) "
          f"over {total_tok:,} tokens / {nwin_all:,} windows\n")
    ...
    for p in prompts:
        ...
        print(f"PROMPT: {p}")
        print(f"-> {text}")
    return {"val_ppl": ppl, "val_tokens": total_tok, "windows": nwin_all}
...
@app.local_entrypoint()
def pretrain_smoke():
    pretrain.remote(epochs=2.0, smoke_steps=30)


@app.local_entrypoint()
def pretrain_run(epochs: float = 2.0, compile: bool = True):
    pretrain.remote(epochs=epochs, smoke_steps=0, compile=compile)
