...
import { CHAT_MODELS, CHAT_PRESETS, UNIVERSAL_URL } from "@/app/lib/model";
...
export default function Chat() {
  ...
  const canBrowser = CHAT_MODELS.find((m) => m.id === modelId)?.browser ?? false;
  ...
  async function send(text: string) {
    ...
    // ---- in-browser (transformers.js) — only our 125M SFT ----
    if (mode === "browser" && canBrowser) {
      try {
    ...
    try {
      const res = await fetch(`${UNIVERSAL_URL}/chat`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ model_id: modelId, message, max_new_tokens: 200, temperature: 0.7 }),
        signal: ctrl.signal,
      });
  ...
  return (
    <div className="paper-card" style={{ padding: "clamp(1.1rem, 2.5vw, 1.75rem)", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: "1rem", flexWrap: "wrap", marginBottom: "0.55rem" }}>
        <span className="eyebrow">Ask the SFT assistant</span>
        <div style={{ display: "flex", alignItems: "center", gap: "0.8rem" }}>
          <div className="seg">
            {CHAT_MODELS.map((m) => (
              <button key={m.id} data-active={modelId === m.id} onClick={() => setModelId(m.id)} disabled={busy}>{m.label}</button>
            ))}
          </div>
          {messages.length > 0 && (
      ...
      {/* secondary: where it runs (only the 125M can run in the browser) */}
        ...
        {canBrowser ? (
          <>
            ...
            <span className="mono" style={{ fontSize: "0.7rem", color: "var(--faint)" }}>
              {mode === "server" ? "hosted GPU" : "runs in your browser"}
            </span>
          </>
        ) : (
          <span className="mono" style={{ fontSize: "0.7rem", color: "var(--faint)" }}>
            runs on a hosted GPU · scale-to-zero, wakes on first message
          </span>
        )}
      </div>
      ...
      <p style={{ marginTop: "0.9rem", fontSize: "0.8rem", color: "var(--faint)", lineHeight: 1.6 }}>
        Three SFT models on the same Q&amp;A data: our 125M and 500M (full fine-tune) and Gemma 2B
        (QLoRA). The smaller ones will confidently invent case names and figures; the bigger ones are
        more fluent. Not legal or financial advice.
      </p>
    </div>
  );
}
